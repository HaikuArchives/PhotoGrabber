...
/*debug*/
static void
ptp_pack_debug (PTPParams *params, const char *format, ...)
{  
        va_list args;
...
ptp_unpack_string(PTPParams *params, unsigned char* data, uint16_t offset, uint8_t *len)
{
	...
	ptp_pack_debug (params, "PTP: Unpack string.");
	...
	*len = length;
	...
	ptp_pack_debug (params, "PTP: iconv.");
	//nconv = iconv(params->cd_ucs2_to_locale, &src, &srclen, &dest, &destlen);
	ptp_pack_debug (params, "PTP: iconv done.");
