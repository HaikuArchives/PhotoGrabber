...
//
//		BeCam_PrefSaver :: Constructor
BeCam_PrefSaver::BeCam_PrefSaver(const char *name,const int no_of_fields)
{
	BPath prefsPath;
...
bool BeCam_PrefSaver::GetLine(BEntry *_file,long line_number,char *buffer) //buffer must be 256 in size
...
int BeCam_PrefSaver::Get_Position(const char *f)
{
...
void BeCam_PrefSaver::AddField(const char *f,const char *entry)
{
...
bool BeCam_PrefSaver::GetString(const char *f,char *entry,const unsigned short buf_size)
{
	for (int i=0;i<current_number;i++)
	{
		if (strcmp(f,field[i])==0)
...
bool BeCam_PrefSaver::SaveFile(void)
{
...
BeCam_PrefSaver::~BeCam_PrefSaver()
{	//delete arrays
	for(int i=0;i<current_number;i++)
	{
		delete values[i];
